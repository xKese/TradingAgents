...
def get_fundamentals(
    ...
    Failures propagate as exceptions so the vendor router can log them and
    fall back to the next vendor; returning error prose here would count as
    a successful result (no fallback, no NO_DATA sentinel, day-long cache).
    """
...
def get_balance_sheet(
    ...
    Failures propagate as exceptions so the vendor router can log them and
    fall back to the next vendor (see get_fundamentals).
    """
    canonical = normalize_symbol(ticker)
    ticker_obj = yf.Ticker(canonical)
