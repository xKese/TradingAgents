...
class TestDeferredReflection:
    ...
    def test_resolve_benchmark_german_listings(self):
        """Canonicalized German symbols (.DE XETRA / .F Frankfurt) get the DAX
        instead of the SPY fallback (uses the real default benchmark_map)."""
        from tradingagents.default_config import DEFAULT_CONFIG
        mock_graph = MagicMock(spec=TradingAgentsGraph)
        mock_graph.config = {"benchmark_ticker": None,
                             "benchmark_map": DEFAULT_CONFIG["benchmark_map"]}
...
class TestLegacyRemoval:
    ...
    def test_full_pipeline_no_regression(self, tmp_path):
        ...
        # Bind the real _run_graph (and the memory helpers it delegates to) so
        # propagate's call executes the actual write path instead of the
        # auto-MagicMock.
        mock_graph._run_graph = functools.partial(
            TradingAgentsGraph._run_graph, mock_graph
        )
        mock_graph._memory_on = functools.partial(
            TradingAgentsGraph._memory_on, mock_graph
        )
        mock_graph.prepare_run_context = functools.partial(
            TradingAgentsGraph.prepare_run_context, mock_graph
        )
