...
class ConditionalLogic:
    ...
    def should_continue_operational(self, state: AgentState):
        """Run the operational evidence tool once, then finalize the report."""
        messages = state["messages"]
        last_message = messages[-1]
        if last_message.tool_calls:
            return "tools_operational"
        return "Msg Clear Operational"
